...
const createLocation = asyncHandler(async (requ, res) => {
  ...
  return res
    .status(201)
    .json(new apiResponse(200, location, "Location created successfully"));
});
...
const getDeviceLocation = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new apiResponse(200, location, "Device location fetched successfully"),
    );
});
...
const updateDeviceLocation = asyncHandler(async (req, res) => {
  ...
  const updatedLocation = await Device.findByIdAndUpdate(
    deviceId,
    {
      $set: {
        location: {
  ...
  return res
    .status(200)
    .json(new apiError(200, updatedLocation, "Location updated successfully"));
});
  ...
  return res
    ...
    .json(
      new apiResponse(
        200,
        locations,
        "Device location history fetched successfully",
      ),
    );
});
...
const getNearbyDevices = asyncHandler(async (req, res) => {
  ...
  if (nearbyDevices.length === 0) {
    return res
      .status(200)
      .json(new apiResponse(200, null, "No nearby devices found"));
  }

  return res
    .status(200)
    .json(
      new apiResponse(200, nearbyDevices, "These are all the nearby devices"),
    );
});
...
const deleteLocationHistory = asyncHandler(async (req, res) => {
  ...
  if (device.usedBy.toString() !== userId.toString()) {
    throw new apiError(
      403,
      "You are not authorized to delete this device's location history",
    );
  }
  ...
  return res
    ...
    .json(
      new apiResponse(
        200,
        null,
        "Device Location history deleted successfully",
      ),
    );
});
